/*
 * The import tables the bionic ELF loader binds into libkatamari.so.
 *
 * The native library was built for Android, so its shared-object names and
 * ABI do not match the Linux host. Each imported table is answered here by a
 * host implementation or by one of the generated bionic thunks.
 *
 ...
 *
 * Katamari declares the Android system libraries it was linked against. The
 * entries below are treated as builtin so the loader does not look for Android
 * copies inside the extracted game directory.
 *
